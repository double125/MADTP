...
class BLIP_Retrieval(nn.Module):
    def __init__(self,                 
        ...
        self.visual_encoder_m, vision_width = create_vit(vit, image_size, evaluate=evaluate, sd_dim=self.sd_dim)    
        self.vision_proj_m = nn.Linear(vision_width, embed_dim)
        med_config_m = med_config
        med_config_m.search = False
        self.text_encoder_m = BertModel(config=med_config_m, add_pooling_layer=False, sd_dim=self.sd_dim)   
    ...
    def forward(self, image, caption, alpha, idx, temperature=0, train=True):
        with torch.no_grad():
        ...
        text_output, sd_txt_ft = self.text_encoder(text.input_ids, attention_mask = text.attention_mask,                      
        ...
        with torch.no_grad():
            ...
            image_embeds_m, sd_img_ft_m = self.visual_encoder_m(image, space_dict=self.space_dict, temperature=temperature) 
            image_feat_m = F.normalize(self.vision_proj_m(image_embeds_m[:,0,:]),dim=-1)  
            image_feat_m_all = torch.cat([image_feat_m.t(),self.image_queue.clone().detach()],dim=1)                   
            
            text_output_m, sd_txt_ft_m = self.text_encoder_m(text.input_ids, attention_mask = text.attention_mask,                      
                                                return_dict = True, mode = 'text', space_dict=self.space_dict, temperature=temperature)    
        ...
        if temperature !=0 and sd_img_ft is not None and sd_txt_ft is not None and train:
            #l2 normalization
        ...
        if temperature !=0 and sd_img_ft_m is not None and sd_txt_ft_m is not None and train:
        ...
        return loss_ita, loss_itm, loss_fdt, loss_fdt_m
