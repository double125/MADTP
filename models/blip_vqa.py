...
class BLIP_VQA(nn.Module):
    def __init__(self,                 
        ...
        self.text_encoder = BertModel(config=encoder_config, add_pooling_layer=False, sd_dim=self.sd_dim) 
        
        decoder_config = BertConfig.from_json_file(med_config)
        decoder_config.evaluate = evaluate
        self.text_decoder = BertLMHeadModel(config=decoder_config, sd_dim=self.sd_dim)   


    def forward(self, image, question, answer=None, temperature=0, train=True, n=None, weights=None,  inference='rank', k_test=128):
        ...
        if train:               
            ...
            question_output, sd_txt_ft = self.text_encoder(question.input_ids, 
            ...
            loss_vqa = weights * answer_output.loss
            loss_vqa = loss_vqa.sum()/image.size(0)
